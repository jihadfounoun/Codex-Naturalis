...
class InitialCardTest {
    ...
    @BeforeEach
    void setUp() {
        initialCard=new InitialCard("4",new Corner[]{new ResCorner(Resource.FUNGI), new Corner(false, true), new ResCorner(Resource.FUNGI), new Corner(true, false)}
    ...
    @Test
    void testEquals() {
        InitialCard otherCard = new InitialCard("4",new Corner[]{new ResCorner(Resource.FUNGI), new Corner(false, true), new ResCorner(Resource.FUNGI), new Corner(true, false)}
                ,new Corner[]{new ResCorner(Resource.FUNGI), new ResCorner(Resource.ANIMAL), new ResCorner(Resource.INSECT), new ResCorner(Resource.PLANT)},
                new ArrayList<>(Arrays.asList(Resource.FUNGI, Resource.FUNGI, Resource.PLANT))
