...
public class ResourceGoalTest {
    Player player;
    ResourceGoal fungiGoal;
    ResourceGoal animalGoal;
    ResourceGoal insectGoal;
    ResourceGoal plantGoal;

    @BeforeEach
    void setUp() {
        player = new Player("Player8");

        fungiGoal = new ResourceGoal("95", FUNGI);
        animalGoal = new ResourceGoal("97", ANIMAL);
        insectGoal = new ResourceGoal("98", INSECT);
        plantGoal = new ResourceGoal("96", PLANT);

        Corner[] redCorner = new Corner[4];
        redCorner[0] = new ResCorner(FUNGI);
        redCorner[1] = new ResCorner(ANIMAL);
        redCorner[2] = new ResCorner(FUNGI);
        redCorner[3] = new Corner(false, false);

        Corner[] blueCorner = new Corner[4];
        blueCorner[0] = new ResCorner(ANIMAL);
        blueCorner[1] = new Corner(false, false);
        blueCorner[2] = new ResCorner(ANIMAL);
        blueCorner[3] = new ResCorner(FUNGI);

        Corner[] purpleCorner = new Corner[4];

        purpleCorner[0] = new Corner(false, false);
        purpleCorner[1] = new ResCorner(INSECT);
        purpleCorner[2] = new ResCorner(PLANT);
        purpleCorner[3] = new ResCorner(INSECT);

        Corner[] greenCorner = new Corner[4];
        greenCorner[0] = new ResCorner(INSECT);
        greenCorner[1] = new ResCorner(PLANT);
        greenCorner[2] = new Corner(false, false);
        greenCorner[3] = new ResCorner(PLANT);
    ...
    @Test
    void testUpdate() {
        int[] expected = {6, 6, 8, 8};

        assertEquals(expected[0], fungiGoal.checkPoints(player.getPlayerBoard()));
        assertEquals(expected[1], animalGoal.checkPoints(player.getPlayerBoard()));
        assertEquals(expected[2], insectGoal.checkPoints(player.getPlayerBoard()));
        assertEquals(expected[3], plantGoal.checkPoints(player.getPlayerBoard()));
    }
}
